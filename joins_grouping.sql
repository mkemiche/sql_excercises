...
select p.`productLine` as 'Product Line', od.`quantityOrdered` as '# Sold'
...
select CONCAT(e.`firstName`,'  ', e.`lastName`) as 'Sales Rep', od.`quantityOrdered` as '# Orders', (od.`quantityOrdered` * od.`priceEach`) as 'Total Sales'
